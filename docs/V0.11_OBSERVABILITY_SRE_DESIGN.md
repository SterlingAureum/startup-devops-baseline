...
offline implementation and still-unclaimed local/AWS checks are documented in
...
flowchart TD
    ...
    A --> C[Alloy telemetry collection]
    B --> D[Prometheus and Alertmanager]
    C --> E[Loki and OTLP trace backend]
...
| Traces | OpenTelemetry SDK and OTLP through Alloy or an upstream-compatible Collector | Application code is not coupled to Tempo, Jaeger, X-Ray, or a SaaS backend |
...
release-only PR. v0.11.7 may add an approval-protected, read-only aws-prod
...
| v0.11.3 | Dashboards and recording rules |
| v0.11.4 | Actionable alerts, routing, inhibition, Runbooks, and drills |
| v0.11.5 | Centralized logs and minimal extensible tracing path |
| v0.11.6 | SLOs, error budgets, burn-rate alerts, and SLI-based Rollout gates |
| v0.11.7 | Dev/test/prod observability qualification boundaries |
| v0.11.8 | Clean-room live end-to-end acceptance and closure evidence |
...
v0.11.8 performs one clean-room, build-once release through live aws-dev,
