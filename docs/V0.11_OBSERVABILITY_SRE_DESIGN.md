...
data source. v0.11.6.1.2.1 repairs its WaitForFirstConsumer sync-wave deadlock,
historical exact-name validation, and unbounded Argo CD sync wait. Tracing
remains deferred to v0.11.6.2. v0.11.6.1.2.2 then repairs only the temporary
Event MicroTime precision used by live acceptance. v0.11.6.1.3 closes structured logging
with ordered end-to-end execution, strict source cleanup, retained Loki
history, stage diagnostics, and a consecutive two-run proof. v0.11.6.2.0 implements
the application-side OpenTelemetry contract with W3C propagation, controlled
HTTP and PostgreSQL spans, shared release identity, and real JSON correlation.
OTLP remains disabled until the private Collector and Tempo runtime is added.
v0.11.6.2.1 implements that private local transport and store as one traces-only
Collector Gateway and one repository-owned Tempo 3.0.3 Monolithic Deployment.
Synthetic OTLP acceptance proves transport, query, and Collector replacement
history without enabling demo-api export or claiming local PostgreSQL tracing.
v0.11.6.2.1.1 repairs the acceptance client to emit the hexadecimal identifier
encoding required by OTLP/JSON, retain bounded HTTP rejection details, and
collect explicit diagnostics for both runtime components. The runtime topology
and versions do not change. v0.11.6.2.2 joins the accepted halves by enabling
the existing exporter only in the local App-of-Apps, proving one real HTTP
SERVER span and its Loki JSON correlation, and provisioning a private Grafana
Tempo data source plus a query-time Loki derived field. Trace identifiers remain
unindexed and PostgreSQL live tracing remains deferred.
...
| v0.11.6 | Centralized logs and minimal extensible tracing path; v0.11.6.0 defines the architecture, v0.11.6.1.0 implements bounded demo-api JSON logging, v0.11.6.1.1 adds private local Loki plus Alloy Pod-log collection, v0.11.6.1.2 adds singleton Kubernetes Events plus the Grafana Loki data source, v0.11.6.1.2.1 repairs the WaitForFirstConsumer reconciliation boundary, v0.11.6.1.2.2 repairs Event MicroTime acceptance precision, v0.11.6.1.3 closes local structured logging, v0.11.6.2.0 adds the disabled-by-default demo-api OpenTelemetry contract, v0.11.6.2.1 adds a private local OTel Collector plus Tempo runtime while export remains disabled, v0.11.6.2.1.1 repairs synthetic OTLP/JSON acceptance encoding and diagnostics, v0.11.6.2.2 enables local real-request export with Loki-to-Tempo correlation, v0.11.6.2.3 closes it with ordered read-only acceptance and negative boundaries, and v0.11.6.2.3.1 rejects a healthy but capability-stale runtime artifact before live correlation |
...
## v0.11.7 SLO governance

v0.11.7.0 starts SLO governance without changing delivery decisions. The
reference demo-api objective counts only `GET /version`, treats `5xx` as an
availability failure, targets 99.9% availability, and targets 99% of requests
at or below 500ms over a 30-day rolling window. Health, readiness, and metrics
routes are excluded so platform probes cannot inflate the SLI.

The foundation publishes release-correlated SLI, SLO, and remaining-budget
recording rules plus an immutable Grafana view. It adds no burn-rate alert and
does not modify Argo Rollouts. v0.11.7.1 owns multi-window alerting;
v0.11.7.2 owns the reviewed Rollout gate; v0.11.7.3 owns closure.
