...
variable "environment" {
  ...
  validation {
    ...
    error_message = "The test root must use environment = test so its state cannot claim another environment's resource names."
  }
}
...
variable "vpc_cidr" {
  description = "IPv4 CIDR block assigned to the test VPC."
  type        = string
  default     = "10.30.0.0/16"
}

variable "availability_zones" {
  description = "Availability Zones used by the test environment."
  type        = list(string)
...
variable "single_nat_gateway" {
  description = "Use one shared NAT Gateway in test to reduce cost."
  type        = bool
  default     = true
}

variable "eks_cluster_version" {
  description = "Pinned Kubernetes version for the EKS test environment."
  type        = string
  default     = "1.36"
}
...
variable "eks_enabled_cluster_log_types" {
  description = "EKS control-plane logs sent to CloudWatch. The disposable test profile defaults to no ingestion; use the guarded logging-profile script for production-parity validation."
  type        = list(string)
  default     = []
...
variable "demo_api_hostname" {
  ...
  type        = string
  default     = "demo.test.aureumstack.com"
}
...
variable "cnpg_backup_force_destroy" {
  description = "Allow Terraform destroy to remove all objects and versions from the aws-test CloudNativePG backup bucket."
  type        = bool
  default     = true
}

variable "external_secrets_recovery_window_in_days" {
  description = "Secrets Manager recovery window for the ephemeral aws-test environment."
  type        = number
  default     = 7
...
variable "enable_github_actions_runtime_identity" {
  description = "Map the persistent aws-test GitHub runtime role into this EKS cluster."
  type        = bool
  default     = false
}

variable "github_actions_runtime_role_arn" {
  description = "Persistent account-bootstrap aws-test runtime role ARN; required when runtime access is enabled."
  type        = string
  default     = null
  nullable    = true
  ...
  validation {
    ...
    error_message = "Enable runtime access only with the persistent aws-test runtime role ARN."
  }
}
