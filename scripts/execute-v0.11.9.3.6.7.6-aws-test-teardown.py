#!/usr/bin/env python3
"""Explicitly reviewed staged aws-test cleanup; never retry failed mutations."""
import argparse
...
p=h.p
spec=importlib.util.spec_from_file_location('cleanup_observer',h.ROOT/'scripts/observe-v0.11.9.3.6.7.5.2.1-aws-test-cleanup.py')
o=importlib.util.module_from_spec(spec);spec.loader.exec_module(o)
CONTRACT='delivery/contracts/v0.11.9.3.6.7.6-aws-test-guarded-teardown.json'
...
def make_plan(r,args,record,stage):
    ...
    proof={'status':'aws-test-'+stage+'-delete-plan-produced','stage':stage,'main':args.expected_main,
...
def apply_plan(r,args,record,stage):
    proof=h.read_private(args.proof);p.require(p.digest(args.proof)==args.expected_proof_sha256,'approved-plan-proof-hash')
    ...
    if stage=='eks':
        ...
        p.write(args.workspace/'eks-complete.json',{'main':args.expected_main,'state_sha256':r.state_hash,'runtime_record_sha256':p.digest(args.workspace/'runtime-record.json')})
    else:
...
def main():
    ...
    try:
        c=source_checks();confirmations(args.phase,os.environ);p.exact_main(args.expected_main,c['implementationBaselineCommit'])
        ...
        if args.phase=='verify-runtime':
            ...
            proof={'status':'aws-test-runtime-cleanup-inputs-verified','main':args.expected_main,'created_at_utc':p.timestamp(),
            ...
            proof=h.read_private(args.proof);p.require(p.digest(args.proof)==args.expected_proof_sha256,'proof-hash')
            ...
            if args.phase=='execute-runtime':
                ...
                receipt={'main':args.expected_main,'state_sha256':r.state_hash,'runtime_record_sha256':p.digest(args.workspace/'runtime-record.json')}
        ...
        result.update(control_plane_commit=args.expected_main,cleanup_complete_by_utc=c['cleanupCompleteByUtc'],automatic_retry_performed=False)
