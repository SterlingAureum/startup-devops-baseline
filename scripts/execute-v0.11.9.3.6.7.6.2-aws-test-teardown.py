#!/usr/bin/env python3
"""Read-only verification and separately approved partial cleanup continuation."""
import argparse
...
CONTRACT='delivery/contracts/v0.11.9.3.6.7.6.2-aws-test-runtime-cleanup-resume.json'
TF=h.ROOT/'infra/terraform/aws/environments/test'
AWS=o.AWS
PHASES={'verify-resume':None,'execute-resume':('CONFIRM_AWS_TEST_RUNTIME_RESUME','cleanup-reviewed-aws-test-runtime-resume-once'),
...
def confirmations(phase,environment):
    ...
    p.require('CONFIRM_AWS_TEST_RUNTIME_CLEANUP' not in environment,'old-runtime-confirmation-forbidden')
...
class Runner(o.e.Runner):
    ...
    def delete(self,label,kind,name,namespace=None):
        cmd=self.kube+['delete',kind,name,'--ignore-not-found=true','--wait=false','--cascade='+('orphan' if kind=='application' else 'background'),'-o','name']
        if namespace:cmd+=['-n',namespace]
        return self.invoke(label,cmd,mutation=True,json_output=False)
...
def paused_spec(value):
    result=json.loads(json.dumps(value))
    policy=result.get('syncPolicy',{})
    p.require(policy.get('automated') is None,'application-not-paused')
    policy.pop('automated',None)
    if not policy:result.pop('syncPolicy',None)
    return result


def expected_paused(value):
    result=json.loads(json.dumps(value))
    if 'syncPolicy' in result:
        result['syncPolicy'].pop('automated',None)
        if not result['syncPolicy']:result.pop('syncPolicy')
    return result


def journal_signature(journal,record):
    order=[p.APP]+[x for x in record['apps'] if x!=p.APP]
    expected=['freeze-'+x for x in order]+['orphan-'+p.APP,'remove-app-'+p.APP]
    p.require(len(journal)==21 and [x['stage'] for x in journal]==expected,'prior-journal-stage-drift')


def prior_evidence(args,c):
    p.require(args.prior_workspace and args.prior_output and args.bundle and args.deployment_result,'prior-evidence-required')
    for path in (args.prior_workspace,args.prior_output):p.private(path,0o700);p.persistent(path)
    pins=c['priorFailure']['artifacts']
    mapping={'runtimeRecord':args.prior_workspace/'runtime-record.json','reviewedProof':args.prior_workspace/'runtime-proof-reviewed.json',
        'attemptMarker':args.prior_workspace/'runtime-attempt.json','failure':args.prior_output/'failure.json',
        'mutationJournal':args.prior_output/'mutation-attempts.jsonl'}
    for key,path in mapping.items():
        p.private(path);p.require(p.digest(path)==pins[key]['sha256'],'prior-evidence-bytes')
    original=h.read_private(mapping['runtimeRecord']);proof=h.read_private(mapping['reviewedProof'])
    p.require(proof['human_reviewed'] is True and proof['runtime_record_sha256']==p.digest(mapping['runtimeRecord']) and
        proof['state_sha256']==p.digest(h.ROOT/p.STATE) and proof['main']==c['implementationBaselineCommit'],'prior-proof-history-binding')
    journal=[json.loads(x) for x in mapping['mutationJournal'].read_text().splitlines()];journal_signature(journal,original)
    failed=list(args.prior_output.glob('*-remove-app-'+p.APP+'.stderr'));p.require(len(failed)==1,'prior-error-log-required')
    p.private(failed[0]);p.private(failed[0].with_suffix('.stdout'))
    p.require(p.digest(failed[0])==pins['deleteStderr']['sha256'] and failed[0].with_suffix('.stdout').stat().st_size==0,'prior-delete-error-binding')
    historical=p.load(h.ROOT/o.CONTRACT);root,source_map=o.reviewed_bundle(args.bundle,historical);o.deployment_proof(args.deployment_result,historical)
    p.require(original['app_specs'][p.APP]==root['spec'] and set(original['apps'])=={x['name'] for x in source_map}|{p.APP},'original-application-scope')
    uids={}
    for name in original['apps']:
        logs=list(args.prior_output.glob('*-freeze-'+name+'.stdout'));p.require(len(logs)==1,'prior-freeze-response-required')
        app=h.read_private(logs[0]);p.require(app['metadata']['name']==name and app['metadata'].get('uid') and
            paused_spec(app['spec'])==expected_paused(original['app_specs'][name]),'prior-freeze-response-drift')
        uids[name]=app['metadata']['uid']
    logs=list(args.prior_output.glob('*-orphan-'+p.APP+'.stdout'));p.require(len(logs)==1,'prior-root-orphan-response-required')
    app=h.read_private(logs[0]);p.require(app['metadata']['uid']==uids[p.APP] and not app['metadata'].get('finalizers'),'prior-root-orphan-response')
    return original,uids


def resume_apps(items,original,uids):
    apps={x['metadata']['name']:x for x in items}
    expected=set(original['apps']);p.require(len(apps)==len(items) and set(apps) in (expected,expected-{p.APP}),'unexpected-resume-application-set')
    for name,app in apps.items():
        p.require(app['metadata']['uid']==uids[name],'application-recreated-stop')
        p.require(not app['metadata'].get('deletionTimestamp'),'application-deletion-in-progress-stop')
        p.require(paused_spec(app['spec'])==expected_paused(original['app_specs'][name]),'paused-application-source-drift')
        ...
        if name==p.APP:p.require(not app['metadata'].get('finalizers'),'root-finalizer-restored-stop')
    return apps


def scope(r,args,c):
    original,uids=prior_evidence(args,c)
    p.require(original['account']==r.inputs['aws_account_id'] and original['management_cidr']==r.inputs['management_cidr'],'prior-account-cidr')
    values=state();outputs={k:v['value'] for k,v in values['outputs'].items()};h.validate_outputs(outputs,r.inputs['aws_account_id'])
    p.require(outputs==original['outputs'] and addresses(values)==original['managed'],'prior-state-resource-binding')
    inventory=r.invoke('resume-clusters',AWS+['eks','list-clusters'])['clusters']
    p.require(set(inventory)&{p.CLUSTER,'startup-devops-baseline-dev','startup-devops-baseline-prod'}=={p.CLUSTER},'rehearsal-inventory')
    cluster=r.invoke('resume-eks',AWS+['eks','describe-cluster','--name',p.CLUSTER])['cluster'];network=cluster['resourcesVpcConfig']
    p.require(cluster['status']=='ACTIVE' and network['vpcId']==outputs['vpc_id'] and network['endpointPublicAccess'] and network['endpointPrivateAccess'] and
        network['publicAccessCidrs']==[r.inputs['management_cidr']] and network['clusterSecurityGroupId']==original['cluster_sg'],'eks-target-or-cidr-drift')
    config=r.invoke('resume-kubeconfig',r.kube+['config','view','--minify','-o','json']);p.kube_auth(config,cluster['endpoint'])
    r.get('resume-api','namespace','kube-system')
    apps=resume_apps(r.get('resume-apps','applications.argoproj.io','','argocd')['items'],original,uids)
    pools=r.get('resume-pools','nodepools','')['items'];classes=r.get('resume-classes','ec2nodeclasses','')['items']
    p.require({x['metadata']['name']:x['spec'] for x in pools}==original['nodepools'] and len(pools)==2,'nodepool-spec-drift')
    p.require({x['metadata']['name']:x['spec'] for x in classes}==original['nodeclasses'] and len(classes)==3,'nodeclass-spec-drift')
    for ns in ('startup-apps','data-platform','observability'):r.get('resume-ns-'+ns,'namespace',ns)
    pvs=r.get('resume-pvs','pv','')['items'];pvmap={x['metadata']['name']:x for x in pvs}
    p.require(len(pvs)==5 and set(pvmap)=={x['metadata']['name'] for x in original['pvs']},'pv-set-drift')
    for pv in original['pvs']:
        current=pvmap[pv['metadata']['name']]
        p.require(current['metadata']['uid']==pv['metadata']['uid'] and current['spec']['csi']==pv['spec']['csi'] and
            current['spec']['claimRef']==pv['spec']['claimRef'],'captured-pv-identity-drift')
    pvcs=r.invoke('resume-pvcs',r.kube+['get','pvc','-A','-o','json'])['items']
    p.require(len(pvcs)==5 and {(x['metadata']['namespace'],x['metadata']['name']) for x in pvcs}==
        {(x['spec']['claimRef']['namespace'],x['spec']['claimRef']['name']) for x in pvs},'pvc-scope-drift')
    claims=r.get('resume-claims','nodeclaims','')['items'];newids={x['status']['providerID'].rsplit('/',1)[-1] for x in claims}
    p.require(len(claims)==4 and newids==set(original['new_instance_ids']),'nodeclaim-instance-drift')
    cloud=r.invoke('resume-compute',AWS+['ec2','describe-instances','--filters','Name=vpc-id,Values='+outputs['vpc_id']])
    live=[x for group in cloud['Reservations'] for x in group['Instances'] if x['State']['Name'] not in ('terminated','shutting-down')]
    p.require(len(live)==8 and {x['InstanceId'] for x in live}==set(original['instance_ids']),'compute-membership-drift')
    ...
    for instance in live:
        ...
        p.require(instance['State']['Name']=='running' and not instance.get('InstanceLifecycle') and instance['InstanceType']==('c6i.large' if new else 't3.medium'),'compute-cost-envelope-drift')
        if not new:p.require(any(t['Key']=='aws:autoscaling:groupName' and t['Value'] in groups for t in instance.get('Tags',[])),'system-node-association')
    ...
    p.require(diskids==set(original['disk_ids']) and len(diskids)==13,'captured-disk-association-drift')
    vols=r.invoke('resume-volumes',AWS+['ec2','describe-volumes','--filters','Name=attachment.instance-id,Values='+','.join(original['instance_ids'])])['Volumes']
    p.require({x['VolumeId'] for x in vols}==diskids and len(vols)==13 and sum(x['Size'] for x in vols)==312,'attached-volume-cost-drift')
    lbs=r.invoke('resume-alb',AWS+['elbv2','describe-load-balancers'])['LoadBalancers'];target=[x for x in lbs if x['VpcId']==outputs['vpc_id']]
    p.require(len(target)==1 and target[0]['LoadBalancerArn']==original['alb_arn'],'alb-membership-drift')
    tags=r.invoke('resume-alb-tags',AWS+['elbv2','describe-tags','--resource-arns',original['alb_arn']]);tagmap={x['Key']:x['Value'] for x in tags['TagDescriptions'][0]['Tags']}
    ...
    records=r.invoke('resume-alias',AWS+['route53','list-resource-record-sets','--hosted-zone-id',original['zone']])['ResourceRecordSets']
    alias=[x for x in records if x['Name']=='demo.test.aureumstack.com.' and x['Type'] in ('A','AAAA','CNAME')]
    p.require(alias==[original['alias']],'exact-test-alias-drift')
    metadata=r.invoke('resume-container',AWS+['secretsmanager','describe-secret','--secret-id',outputs['external_secrets_secret_arn']])
    p.require(metadata['ARN']==outputs['external_secrets_secret_arn'] and not metadata.get('DeletedDate') and
        any('AWSCURRENT' in x for x in metadata.get('VersionIdsToStages',{}).values()),'credential-metadata-drift')
    backups=r.invoke('resume-backup-inventory',AWS+['s3api','list-object-versions','--bucket',outputs['cnpg_backup_bucket_name'],'--expected-bucket-owner',r.inputs['aws_account_id']])
    result=dict(original);result['apps']=sorted(apps);result['app_specs']={name:app['spec'] for name,app in apps.items()}
    result['app_uids']={name:app['metadata']['uid'] for name,app in apps.items()};result['root_present']=p.APP in apps
    result['prior_runtime_record_sha256']=c['priorFailure']['artifacts']['runtimeRecord']['sha256']
    p.write(args.output_directory/'backup-inventory-summary.json',{'version_count':len(backups.get('Versions',[])),
        'delete_marker_count':len(backups.get('DeleteMarkers',[])),'recorded_bytes':sum(x['Size'] for x in backups.get('Versions',[])),'atomic':False})
    return result
...
def runtime_cleanup(r,record):
    # Continue only the remaining Applications. Never repeat completed freeze patches.
    order=([p.APP] if p.APP in record['apps'] else [])+[name for name in record['apps'] if name!=p.APP]
    for name in order:
        app=r.get('resume-delete-check-'+name,'application',name,'argocd')
        p.require(app['metadata']['uid']==record['app_uids'][name] and app['spec']==record['app_specs'][name],'application-resume-identity-drift')
        p.require(paused_spec(app['spec'])==paused_spec(record['app_specs'][name]),'application-resume-not-paused')
        p.require(not app['metadata'].get('deletionTimestamp') and app.get('status',{}).get('operationState',{}).get('phase') not in ('Running','Terminating'),'application-operation-in-progress-stop')
        finalizers=set(app['metadata'].get('finalizers',[]));p.require(finalizers<=FINALIZERS,'unknown-finalizer-stop')
        if name==p.APP:p.require(not finalizers,'root-finalizer-restored-stop')
        if finalizers:r.invoke('orphan-remaining-'+name,r.kube+['patch','application',name,'-n','argocd','--type=merge','-p','{"metadata":{"finalizers":[]}}','--output=json'],True)
        r.delete('remove-remaining-app-'+name,'application',name,'argocd')
...
def main():
    ...
    for name in ('inputs','workspace','output-directory','bundle','deployment-result','proof','prior-workspace','prior-output'):parser.add_argument('--'+name,type=Path,required=name in ('inputs','workspace','output-directory'))
    ...
    try:
        ...
        end=p.utc(c['runtimeStopUtc'] if 'resume' in args.phase else c['cleanupCompleteByUtc'])
        p.require((end-h.now()).total_seconds()>=(1200 if 'resume' in args.phase else 900),'insufficient-cleanup-time')
        h.new_directory(args.output_directory)
        if args.phase=='verify-resume':h.new_directory(args.workspace)
        ...
        if args.phase=='verify-resume':
            ...
            proof={'window_contract_sha256':window_digest(),'cost_projection':cost_model,'status':'aws-test-runtime-resume-inputs-verified','main':args.expected_main,'created_at_utc':p.timestamp(),
                ...
                'human_reviewed':False,'teardown_authorized':False,'prior_failure_sha256':c['priorFailure']['artifacts']['failure']['sha256']}
            p.write(args.workspace/'runtime-proof.json',proof);result=proof|{'runtime_resource_inventory_review_required':True,'remaining_application_count':len(record['apps']),'root_present':record['root_present'],'completed_freeze_repeated':False}
        else:
            ...
            if args.phase=='execute-resume':
                ...
                p.write(args.workspace/'runtime-resume-attempt.json',{'proof_sha256':args.expected_proof_sha256,'at_utc':p.timestamp(),'outcome_not_yet_known':True})
                p.require(args.bundle and args.deployment_result,'deployment-evidence-required')
                ...
                result={'status':'aws-test-runtime-resume-cleanup-complete','terraform_command_executed':False,'eks_deleted':False,'completed_freeze_repeated':False,'next_action':'separately-approve-eks-delete-plan'}
