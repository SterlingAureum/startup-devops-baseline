...
CONTRACT = 'delivery/contracts/v0.11.9.3.6.7.6.6-guarded-aws-test-residual-cost-audit.json'
...
class Runner:
    ...
    def invoke(self, label, service, operation, params=(), absent=(), cached=False):
        ...
        self.check()
        if rc:
            codes = re.findall(rb'An error occurred \(([A-Za-z0-9_.-]+)\) when calling the ([A-Za-z0-9]+) operation:', err.read_bytes())
            require(len(codes) == 1 and codes[0][0].decode() in absent and codes[0][1].decode().lower() == operation.replace('-', '') and not out.read_bytes().strip(), 'aws-error-not-absence')
            value = None
