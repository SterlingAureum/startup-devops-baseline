...
_fleet_spec = importlib.util.spec_from_file_location('fleet_audit_rules', ROOT / 'scripts/aws_fleet_audit_classification.py')
fleet_rules = importlib.util.module_from_spec(_fleet_spec)
_fleet_spec.loader.exec_module(fleet_rules)
...
class Runner:
    def __init__(self, output, c, stop, backend=transport, clock=now, state_check=empty_state):
        ...
        self.instant_history_fleets = 0; self.instant_history_instance_ids = set()
...
def tagged_record(r, arn, index, record):
    ...
    if fleet:
        ...
        require(r.compute_verified, 'fleet-native-absence-proof-required')
        if value is None:
            r.terminal_fleets += 1; return
        rows = array(value, 'Fleets')
        require(len(rows) == 1, 'fleet-response-count')
        try:
            category, instance_ids = fleet_rules.classify_fleet(
                rows[0], fleet[1], PROJECT, 'test', CLUSTER)
        except fleet_rules.FleetClassificationError as error:
            raise Stop(str(error)) from None
        if category == 'terminal':
            r.terminal_fleets += 1; return
        captured = set(record['instance_ids'])
        require(set(instance_ids).issubset(captured), 'unplanned-instant-fleet-instance')
        for ordinal, iid in enumerate(instance_ids):
            # Fresh per-ID lookup, even if native_sweep queried this ID earlier.
            current = r.invoke(label + '-instance-' + str(ordinal), 'ec2',
                'describe-instances', ['--instance-ids', iid], ('InvalidInstanceID.NotFound',))
            if current is not None:
                current_rows = instances(current)
                require(all(x.get('InstanceId') == iid and
                    isinstance(x.get('State'), dict) and x['State'].get('Name') == 'terminated'
                    for x in current_rows), 'instant-fleet-instance-not-absent')
        r.instant_history_instance_ids.update(instance_ids)
        r.instant_history_fleets += 1; return
...
def full_audit(r, record):
    ...
    return {'audit_scope':'captured aws-test resources and test/cluster tagged us-east-1 resources; captured global IAM and DNS',
            ...
            'verified_instant_fleet_instance_count':len(r.instant_history_instance_ids),
