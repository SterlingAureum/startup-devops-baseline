...
def main() -> None:
    ...
    require(contract["awsPermissions"]["absentProbeBeforeEnvironmentApply"] is True, "Absent probe ordering changed")
    ...
    role_module = read(root, "infra/terraform/aws/modules/github-actions-runtime-role/main.tf")
    role_variables = read(root, "infra/terraform/aws/modules/github-actions-runtime-role/variables.tf")
    access_module = read(root, "infra/terraform/aws/modules/github-actions-runtime-identity/main.tf")
    access_variables = read(root, "infra/terraform/aws/modules/github-actions-runtime-identity/variables.tf")
    require('"token.actions.githubusercontent.com:aud" = "sts.amazonaws.com"' in role_module, "OIDC audience is not exact")
    require('"token.actions.githubusercontent.com:sub" = local.oidc_subject' in role_module, "OIDC subject is not exact")
    require("repo:*" not in role_module and "StringLike" not in role_module, "OIDC trust uses a wildcard")
    actions = set(re.findall(r'"((?:sts|eks):[A-Za-z*]+)"', role_module))
    ...
    require("Resource = var.cluster_arn" in role_module, "EKS permission is not exact-cluster scoped")
    require('Environment        = "account-bootstrap"' in role_module, "Persistent role is tagged as disposable environment state")
    require("RuntimeEnvironment = local.environment_name" in role_module, "Persistent role runtime tag is missing")
    require('kubernetes_groups = [local.kubernetes_group]' in access_module, "EKS access entry does not map custom RBAC")
    require("principal_arn     = var.runtime_role_arn" in access_module, "EKS access entry does not consume the persistent role")
    require("aws_iam_role" not in access_module and "aws_iam_role_policy" not in access_module, "Environment access module still owns the persistent role")
    require('["aws-dev", "aws-test"]' in role_variables, "Runtime role module permits an unsafe environment")
    require('["aws-dev", "aws-test"]' in access_variables, "Runtime access module permits an unsafe environment")

    persistent_root = read(root, "infra/terraform/aws/runtime-identities/main.tf")
    persistent_variables = read(root, "infra/terraform/aws/runtime-identities/variables.tf")
    persistent_outputs = read(root, "infra/terraform/aws/runtime-identities/outputs.tf")
    require('source   = "../modules/github-actions-runtime-role"' in persistent_root, "Persistent runtime root does not use the role module")
    require('aws-dev  = "dev"' in persistent_root and 'aws-test = "test"' in persistent_root, "Persistent runtime environments changed")
    require("data.aws_caller_identity.current.account_id" in persistent_root, "Persistent role does not derive the exact account")
    require("cluster/${var.project_name}-${environment_name}" in persistent_root, "Persistent role cluster ARN is not deterministic")
    require("github_actions_oidc_provider_arn" in persistent_variables, "Persistent runtime root lacks the account OIDC input")
    require("aws_dev_runtime_role_arn" in persistent_outputs and "aws_test_runtime_role_arn" in persistent_outputs, "Persistent runtime outputs are incomplete")
    require("module.eks" not in persistent_root and "aws_eks_access_entry" not in persistent_root, "Persistent runtime root depends on a live EKS cluster")
    ...
    for environment in ("dev", "test"):
        ...
        require("runtime_role_arn = var.github_actions_runtime_role_arn" in main_tf, f"{environment} does not map the persistent runtime role")
        variables_tf = read(root, f"infra/terraform/aws/environments/{environment}/variables.tf")
        require('variable "github_actions_runtime_role_arn"' in variables_tf, f"{environment} runtime role input missing")
        require("github_actions_oidc_provider_arn" not in variables_tf, f"{environment} still owns the account OIDC role lifecycle")
