...
contract_path = 'delivery/contracts/v0.11.9.3.6.7.7.11-dev-local-cli-preflight.json'
document_path = 'docs/V0.11.9.3.6.7.7.11_DEV_LOCAL_CLI_PREFLIGHT.md'
contract = json.loads((root / contract_path).read_text())
digest = lambda path: hashlib.sha256((root / path).read_bytes()).hexdigest()

assert digest(contract_path) == '6b5a70466197239fa52a4df6e9dc3da13fa1c3d82f16e9d6f303b899d004abdb', 'contract drift'
assert contract['schemaVersion'] == contract['version'] == 'v0.11.9.3.6.7.7.11'
assert contract['predecessor'] == 'v0.11.9.3.6.7.7.10'
assert contract['implementationBaselineCommit'] == '958b959fdfada3449b2f4c710141c1c1ab118c21'
assert contract['status'] == 'dev-local-cli-preflight-implemented'
...
assert len(methods) == len(set(methods)) == contract['testMethodCount'] == 22

tree = ast.parse((root / contract['entryPath']).read_text())
classes = {node.name for node in tree.body if isinstance(node, ast.ClassDef)}
assert classes == {'LocalPreflightStopped', 'SystemUtcClock',
                   'StrictPrivateInputFiles'}
allowed = {
...
assert "entry.dispatch('verify', request)" in source
assert "dispatch('execute'" not in source
assert 'DevConformanceTransport(' not in source
assert "add_argument('execute'" not in source
assert 'FrozenClock((now,))' in source
assert 'FixedPrivateInputReader(inputs)' in source

assert contract['localCliPreflight'] == {
    'commands': ['verify'],
    'confirmation': 'observe-reviewed-aws-dev-command-preflight',
    'phaseChoices': 8,
    'executeCommandImplemented': False,
    'environmentVariableReaderImplemented': False,
    'arbitraryEndpointAccepted': False,
    'systemUtcClockReadCount': 1,
    'successOutput': 'one-canonical-json-line',
    'failureOutput': 'one-redacted-stderr-line',
}
assert contract['offlineAdaptation'] == {
    'clockAdapter': 'one-system-utc-reading-into-frozen-clock',
    'privateInputAdapter': 'two-strict-files-into-fixed-private-input-reader',
    'receiptStore': 'v0.11.9.3.6.7.7.9-local-durable-store-read-only',
    'receiptPrefixValidated': True,
    'receiptWritten': False,
    'dispatch': 'v0.11.9.3.6.7.7.10-verify-only',
    'fixedFakeTransportInvoked': False,
}
assert contract['reviewResult'] == {
    ...
    'receiptRestartPrefixCovered': True,
...
for key in ('systemClockReads', 'localPrivateFileReads',
            'localPrivateReceiptFileReads'):
    assert effects[key] is True
for key in ('localPrivateReceiptFileWrites', 'environmentVariableReads',
            'fakeTransport', 'cloudTransport', 'kubernetesTransport',
...
print('v0.11.9.3.6.7.7.11 source/AST/privacy pins passed; dev local verify-only CLI preflight.')
PYTHON
PYTHONDONTWRITEBYTECODE=1 python3 "$ROOT_DIR/scripts/test-v0.11.9.3.6.7.7.11-dev-local-cli-preflight.py"
bash "$ROOT_DIR/scripts/validate-v0.11.9.3.6.7.7.10-dev-command-entry-conformance.sh"
echo "v0.11.9.3.6.7.7.11 passed; local verify only, no execute command, transport, receipt write or live authority."
